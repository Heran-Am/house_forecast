...
OUT_CSV = OUT_DIR / "walkforward_results.csv"
...
def make_plots(res: pd.DataFrame):
    ...
    # Plot 1: MAE improvement by year
    plt.figure()
    plt.plot(res["test_year"], res["mae_improvement"], marker="o")
    plt.axhline(0, linewidth=1)
    plt.title("Walk-forward: MAE improvement vs Naive (positive = better)")
    plt.xlabel("Test year")
    plt.ylabel("MAE improvement ($)")
    plt.tight_layout()
    plt.savefig(FIG_DIR / "walkforward_mae_improvement.png", dpi=160)

    # Plot 2: RMSE improvement by year
    plt.figure()
    plt.plot(res["test_year"], res["rmse_improvement"], marker="o")
    plt.axhline(0, linewidth=1)
    plt.title("Walk-forward: RMSE improvement vs Naive (positive = better)")
    plt.xlabel("Test year")
    plt.ylabel("RMSE improvement ($)")
    plt.tight_layout()
    plt.savefig(FIG_DIR / "walkforward_rmse_improvement.png", dpi=160)
...
def main():
    ...
    for test_year in test_years:
        ...
        results.append({
            "test_year": test_year,
            "train_through": valid_year - 1,
            "valid_year": valid_year,
            "best_iter": best_iter if best_iter is not None else -1,
            "test_rows": int(len(test_df)),

            "naive_mae": float(mae_naive),
            "model_mae": float(mae_model),
            "mae_improvement": float(mae_naive - mae_model),

            "naive_rmse": float(rmse_naive),
            "model_rmse": float(rmse_model),
            "rmse_improvement": float(rmse_naive - rmse_model),
        })
    ...
    print("\nSummary:")
    print(res[[
        "test_year", "test_rows", "best_iter",
        "mae_improvement", "rmse_improvement",
        "naive_mae", "model_mae", "naive_rmse", "model_rmse"
